    ...
    <nav className={`fixed w-full z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-md py-2' : 'bg-transparent py-4'}`}>
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center">
          <Link to="/" className="text-2xl font-bold text-[#A67C52]">
            RentKar
          </Link>
      ...
      {isMenuOpen && (
        <div className="md:hidden bg-white border-t border-[#D2B48C] py-4 px-4 shadow-lg">
          <div className="flex items-center mb-4">
            ...
            />
            <Search className="absolute left-7 top-[4.2rem] text-[#5D4037] h-5 w-5" />
          </div>
          <div className="flex flex-col space-y-3">
            {navLinks.map((link) => (
              <Link key={link.name} to={link.path} className="block py-2 px-4 rounded hover:bg-[#F5F1E9]">
                {link.name}
              </Link>
            ))}
            <div className="border-t border-[#D2B48C] pt-3 mt-2">
              <Link to="/login" className="block py-2 px-4 text-[#A67C52] font-semibold">Sign In / Register</Link>
            </div>
          </div>
        </div>
      )}
